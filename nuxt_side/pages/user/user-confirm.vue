...
export default {
  ...
  methods: {
    ...
    async submitUser() {
      ...
      userData.created_at = (new Date(Date.now() - (new Date()).getTimezoneOffset() * 60000)).toISOString().substr(0, 10);
      userData.updated_at = (new Date(Date.now() - (new Date()).getTimezoneOffset() * 60000)).toISOString().substr(0, 10);
      ...
      try {
        let response = this.$axios.$post(`/users/`, formData, config);
